...
Drop-in replacement for ``ImageNetDataModule`` that uses NVIDIA DALI for
GPU-pipelined JPEG decode, random-resized-crop, and horizontal flip.
...
class _BatchThreeAugment(torch.nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        grey_mask = choice == 0
        if grey_mask.any():
            grey = images[grey_mask].float().mean(dim=-3, keepdim=True)
            images[grey_mask] = grey.expand_as(images[grey_mask]).to(images.dtype)

        solar_mask = choice == 1
        if solar_mask.any():
            sub = images[solar_mask]
            images[solar_mask] = torch.where(sub >= 128, 255 - sub, sub)
...
class _BatchColorJitter(torch.nn.Module):
    ...
    _LUMA = torch.tensor([0.2989, 0.587, 0.114]).view(1, 3, 1, 1)
    ...
    def _grayscale(self, images: torch.Tensor) -> torch.Tensor:
        luma = self._LUMA.to(device=images.device, dtype=images.dtype)
        return (images * luma).sum(dim=-3, keepdim=True)

    def _apply_brightness(self, images: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        if not mask.any() or self.brightness <= 0:
            return images
        B_sel = mask.sum().item()
        factor = 1.0 + (torch.rand(B_sel, 1, 1, 1, device=images.device) * 2 - 1) * self.brightness
        images[mask] = images[mask] * factor
        return images

    def _apply_contrast(self, images: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        if not mask.any() or self.contrast <= 0:
            return images
        B_sel = mask.sum().item()
        sub = images[mask]
        grey_mean = self._grayscale(sub).mean(dim=(-3, -2, -1), keepdim=True)
        factor = 1.0 + (torch.rand(B_sel, 1, 1, 1, device=images.device) * 2 - 1) * self.contrast
        images[mask] = grey_mean + (sub - grey_mean) * factor
        return images

    def _apply_saturation(self, images: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        if not mask.any() or self.saturation <= 0:
            return images
        B_sel = mask.sum().item()
        sub = images[mask]
        grey = self._grayscale(sub)
        factor = 1.0 + (torch.rand(B_sel, 1, 1, 1, device=images.device) * 2 - 1) * self.saturation
        images[mask] = grey + (sub - grey) * factor
        return images

    def forward(self, images: torch.Tensor) -> torch.Tensor:
        B, dev = images.shape[0], images.device
        ops = [self._apply_brightness, self._apply_contrast, self._apply_saturation]

        perms = torch.stack([torch.randperm(3, device=dev) for _ in range(B)])

        for step in range(3):
            for op_idx in range(3):
                mask = perms[:, step] == op_idx
                if mask.any():
                    images = ops[op_idx](images, mask)
...
class DALIImageNetDataModule(pl.LightningDataModule):
    ...
    DALI handles the heavy lifting: file I/O → GPU JPEG decode →
    RandomResizedCrop → horizontal flip — all pipelined and batched in custom
    CUDA kernels.  Post-DALI augmentations (ThreeAugment, ColorJitter) run as
    batched PyTorch GPU ops on uniform ``(B, 3, H, W)`` tensors.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if imagefolder_dir is None:
            raise ValueError(
                "DALIImageNetDataModule requires imagefolder_dir "
        ...
        self._norm_mean = torch.tensor(mean, dtype=torch.float32).view(1, 3, 1, 1)
        self._norm_std = torch.tensor(std, dtype=torch.float32).view(1, 3, 1, 1)
    ...
    def prepare_data(self) -> None:
        pass  # ImageFolder data assumed to be on disk already
    ...
    def on_before_batch_transfer(self, batch, dataloader_idx):
        ...
        if is_train:
            if self._three_augment is not None:
                images = self._three_augment(images)  # still uint8
            # uint8 → float32 [0, 1]
        ...
        # Normalize
        mean = self._norm_mean.to(device=images.device)
        std = self._norm_std.to(device=images.device)
    ...
    def transfer_batch_to_device(self, batch, device, dataloader_idx):
        return batch  # already on GPU from DALI
