"""Optimized DALI-accelerated ImageNet DataModule — v3.

Builds on the v2 optimised module with additional performance improvements:

1. bf16 augmentation pipeline — halves memory bandwidth for all post-DALI ops
2. Fully compilable ``_BatchThreeAugment`` (blur on full batch +
   ``torch.where``, no graph breaks from boolean-index scatter/gather)
3. Single ``torch.compile``d function covering ColorJitter + normalise
   (ThreeAugment + float conversion + ColorJitter + normalise)
...
# ---------------------------------------------------------------------------
# DALI pipelines (identical to v2 — HWC output)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Thin wrapper — DALI outputs (B, H, W, C) uint8; permute done in PyTorch
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Batched ThreeAugment — uncompiled (gaussian_blur needs random sigma)
# ---------------------------------------------------------------------------

class _BatchThreeAugment(torch.nn.Module):
    """Batched DeiT III 3-Augment on (B, C, H, W) uint8 tensors.

    Uses ``torch.where`` for grayscale/solarize. Blur uses masked indexing
    since ``gaussian_blur`` requires a per-batch random sigma that would
    cause ``torch.compile`` recompilation.
    """
...
# ---------------------------------------------------------------------------
# Compiled bf16 ColorJitter + normalise pipeline
# ---------------------------------------------------------------------------

def _color_jitter_and_normalize_fn(
    images: torch.Tensor,
    luma: torch.Tensor,
    brightness: float,
    contrast: float,
    ...
    """ColorJitter + normalise in bf16: (B,C,H,W) bf16 [0,1] → normalised."""
    if use_color_jitter:
        B = images.shape[0]
        dev = images.device

        b_factor = 1.0 + (torch.rand(B, 1, 1, 1, device=dev, dtype=torch.bfloat16) * 2 - 1) * brightness
        c_factor = 1.0 + (torch.rand(B, 1, 1, 1, device=dev, dtype=torch.bfloat16) * 2 - 1) * contrast
        s_factor = 1.0 + (torch.rand(B, 1, 1, 1, device=dev, dtype=torch.bfloat16) * 2 - 1) * saturation

        perms = torch.rand(B, 3, device=dev).argsort(dim=1)

        for step in range(3):
            grey = (images * luma).sum(dim=1, keepdim=True)
...
_compiled_cj_normalize = torch.compile(_color_jitter_and_normalize_fn)


@torch.compile
def _compiled_val_normalize(
    images_uint8: torch.Tensor,
    inv_std_255: torch.Tensor,
    bias: torch.Tensor,
) -> torch.Tensor:
    """Fused val normalisation: uint8 NCHW → bf16 normalised NCHW."""
    return images_uint8.to(torch.bfloat16) * inv_std_255 + bias
...
class DALIImageNetOptimizedV3DataModule(pl.LightningDataModule):
    """Optimised DALI-accelerated ImageNet datamodule — v3.

    All post-DALI augmentations run in bf16 via a compiled pipeline,
    halving memory bandwidth for ColorJitter + normalise.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if imagefolder_dir is None:
            raise ValueError(
                "DALIImageNetOptimizedV3DataModule requires imagefolder_dir "
        ...
        mean_t = torch.tensor(mean, dtype=torch.bfloat16).view(1, 3, 1, 1)
        std_t = torch.tensor(std, dtype=torch.bfloat16).view(1, 3, 1, 1)
        self._norm_mean = mean_t
        self._norm_std = std_t
        self._luma = torch.tensor([0.2989, 0.587, 0.114], dtype=torch.bfloat16).view(1, 3, 1, 1)
        self._inv_std_255 = (1.0 / (torch.tensor(std, dtype=torch.float32) * 255.0)).to(torch.bfloat16).view(1, 3, 1, 1)
        self._neg_mean_over_std = (-(torch.tensor(mean, dtype=torch.float32) / torch.tensor(std, dtype=torch.float32))).to(torch.bfloat16).view(1, 3, 1, 1)
        ...
        self._use_color_jitter = augment_cfg is not None and augment_cfg.color_jitter > 0
        self._brightness = augment_cfg.color_jitter if self._use_color_jitter else 0.0
        self._contrast = augment_cfg.color_jitter if self._use_color_jitter else 0.0
        self._saturation = augment_cfg.color_jitter if self._use_color_jitter else 0.0

        self._three_augment: Optional[_BatchThreeAugment] = None
        if augment_cfg is not None and augment_cfg.use_three_augment:
    ...
    def _ensure_on_device(self, device: torch.device) -> None:
        ...
        self._luma = self._luma.to(device)
    ...
    # ------------------------------------------------------------------
    # Batch hooks — DALI delivers (B, C, H, W) uint8 already in NCHW
    # ------------------------------------------------------------------
    ...
    def on_before_batch_transfer(self, batch, dataloader_idx):
        ...
        self._ensure_on_device(images.device)

        is_train = self.trainer is not None and self.trainer.training
        if is_train:
            if self._three_augment is not None:
                images = self._three_augment(images)  # uint8 NCHW
            images = images.to(torch.bfloat16) / 255.0
            images = _compiled_cj_normalize(
                images,
                self._luma,
                self._brightness,
                self._contrast,
                ...
                self._norm_mean,
                self._norm_std,
                self._use_color_jitter,
            )
        else:
            images = _compiled_val_normalize(images, self._inv_std_255, self._neg_mean_over_std)
