...
"""EMNIST Spatial Recall 2D (Color Selection) - Hyena + Patchify M.
...
Uses correct L_cache = canvas_size // patch_size for SIREN kernel.
"""
...
def get_config() -> ExperimentConfig:
    """Get the configuration for EMNIST color selection with Hyena + Patchify M."""
    config = spatial_recall_2d_base_experiment_config(
        in_channels=INPUT_CHANNELS,
        out_channels=OUTPUT_CHANNELS,
        hidden_dim=HIDDEN_DIM,
        training_iterations=TRAINING_ITERATIONS,
        wandb_job_group="spatial_recall_2d_emnist_color_selection_m",
    )
