...
"""EMNIST Spatial Recall 2D (Color Conditioning) - Attention + Patchify XS.
...
# Patchification parameters (p=8 is best for Attention based on color_selection results)
PATCH_SIZE = 8
STRIDE = 8
...
def get_config() -> ExperimentConfig:
    """Get the configuration for EMNIST color conditioning with Attention + Patchify XS."""
    config = spatial_recall_2d_base_experiment_config(
        in_channels=INPUT_CHANNELS,
        out_channels=OUTPUT_CHANNELS,
        hidden_dim=HIDDEN_DIM,
        training_iterations=TRAINING_ITERATIONS,
        wandb_job_group="spatial_recall_2d_emnist_color_conditioning_xs",
    )
