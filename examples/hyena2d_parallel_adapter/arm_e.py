...
"""Arm E: full fine-tune + parallel Hyena2D adapter, random placement.

Trainable: everything (attention + adapter + head).
Architecture: ViT5ParallelHyenaSequenceMixer (adapter zero-init at load).

Compared to arm C (frozen attention), this allows the attention to also adapt.
Compared to arm D (no adapter), this adds the Hyena inductive bias.
...
def get_config(seed: int = 0) -> ExperimentConfig:
    """Return the arm E config (full fine-tune + Hyena adapter).

    Args:
        seed: Random seed.

    Returns:
        ExperimentConfig for arm E.
    """
    ...
    config.comment = f"arm_e_full_finetune_parallel_hyena_rank{RANK}"
    return config
