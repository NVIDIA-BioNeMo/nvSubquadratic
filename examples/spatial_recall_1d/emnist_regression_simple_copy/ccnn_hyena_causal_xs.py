...
"""EMNIST Spatial Recall 1D - Hyena XS (Extra-Small) Causal.
...
Hyena with causal convolutions:
- CKConvND with is_causal=True uses causal FFT convolution
- CausalConv1D for short conv (left-only padding)
- Suitable for autoregressive tasks

Model Size: XS (Extra-Small)
- Hidden dim: 160
- SIREN kernel with 3 layers
- Params: ~0.77M (same as 2D)
"""
...
def get_config() -> ExperimentConfig:
    """Get the configuration for EMNIST spatial recall 1D with Hyena XS (causal)."""
    ...
    # Mixer: Hyena with causal convolutions
