...
"""Hyena config for active_matter -- channel-first norms (PyTorch, no QuACK).

Identical to ../hyena.py except norms inside the Hyena mixer use
channel-first layout with ``use_quack=False``, so torch.compile can
fuse the norm with surrounding operations:

- ``pixelhyena_norm``, ``output_norm``: ``RMSNormChannelFirst(use_quack=False)``
- ``qk_norm``: ``L2Norm(dim=1)``

Since active_matter uses circular FFT padding (``torch_fft`` only -- no
``subq_ops`` for circular), the compiler already has full visibility into
the convolution.  This tests whether removing norm fusion barriers helps
when the entire Hyena pipeline is compiler-visible.
"""
...
def get_config() -> ExperimentConfig:
    """Build Hyena config for active_matter with channel-first norms."""
        ...
        block_cfg=LazyConfig(ResidualBlock)(
            sequence_mixer_cfg=LazyConfig(QKVSequenceMixer)(
                ...
                mixer_cfg=LazyConfig(Hyena)(
                    ...
                    # Channel-first norms: tensor is already [B, C, H, W]
                    pixelhyena_norm_cfg=LazyConfig(RMSNormChannelFirst)(
                        dim=NUM_HIDDEN_CHANNELS,
                        use_quack=False,
                    ),
                    output_norm_cfg=LazyConfig(RMSNormChannelFirst)(
                        dim=NUM_HIDDEN_CHANNELS,
                        use_quack=False,
                    ),
