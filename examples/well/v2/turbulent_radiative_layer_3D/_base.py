"""Shared config for turbulent_radiative_layer_3D experiments (v2).

Dataset: turbulent_radiative_layer_3D
    - 3D, 128×128×256, 5 dynamic fields, 0 constant fields
    - Dynamic fields: density, pressure, velocity_{x,y,z}
    - ~72 train trajectories × 101 timesteps
    - Asymmetric 3D resolution
...
# ─── Dataset constants ────────────────────────────────────────────────────────
DATA_DIM = 3
SPATIAL_RESOLUTION = (128, 128, 256)
...
BOUNDARY_CONDITIONS = {"x": "PERIODIC", "y": "PERIODIC", "z": "OPEN"}

N_FIELDS = 5  # density, pressure, velocity_{x,y,z}
N_CONSTANT_FIELDS = 0
IN_CHANNELS = N_STEPS_INPUT * N_FIELDS + N_CONSTANT_FIELDS  # 20
OUT_CHANNELS = N_FIELDS  # 5

# 72 train trajectories × 97 windows = 6,984 samples
SAMPLES_PER_EPOCH = 6_984

# ─── Training constants (shared across models) ───────────────────────────────
TRAINING_ITERATIONS = 110_000
WARMUP_ITERATIONS_PERCENTAGE = 0.05

BATCH_SIZE = 1  # configs/data/turbulent_radiative_layer_3D.yaml
NUM_WORKERS = 12
GRAD_CLIP = 1.0
