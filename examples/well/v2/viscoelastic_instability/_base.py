"""Shared config for viscoelastic_instability experiments (v2).

Dataset: viscoelastic_instability
    - 2D, 512×512, 8 dynamic fields, 0 constant fields
    - Dynamic fields: pressure, c_zz, velocity_{x,y}, C_{xx,xy,yx,yy}
    - ~213 train trajectories × variable timesteps (20–60)
...
BOUNDARY_CONDITIONS = {"x": "PERIODIC", "y": "WALL"}

N_FIELDS = 8  # pressure, c_zz, velocity_{x,y}, C_{xx,xy,yx,yy}
N_CONSTANT_FIELDS = 0
IN_CHANNELS = N_STEPS_INPUT * N_FIELDS + N_CONSTANT_FIELDS  # 32
OUT_CHANNELS = N_FIELDS  # 8

# Variable trajectory lengths (20 or 60 steps). Per-file breakdown:
#   AH: 16 trajs × 16 windows, CAR: 33 × 56, EIT: 28 × 56,
#   transtochaos_EIT_L: 35 × 16, transtochaos_EIT_SAR: 30 × 16,
#   transtononchaos_EIT_L: 31 × 16, transtononchaos_EIT_SAR: 40 × 16
# Total: 5,848 samples
SAMPLES_PER_EPOCH = 5_848

# ─── Training constants (shared across models) ───────────────────────────────
TRAINING_ITERATIONS = 110_000
WARMUP_ITERATIONS_PERCENTAGE = 0.05

BATCH_SIZE = 32  # configs/data/viscoelastic_instability.yaml
NUM_WORKERS = 12
GRAD_CLIP = 1.0
