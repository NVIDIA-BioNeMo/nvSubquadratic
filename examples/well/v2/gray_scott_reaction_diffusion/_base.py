"""Shared config for gray_scott_reaction_diffusion experiments (v2).

Dataset: gray_scott_reaction_diffusion
    - 2D, 128×128, 2 dynamic fields, 0 constant fields
    - Dynamic fields: A, B (chemical concentrations)
    - 960 train trajectories × 1001 timesteps
    - Periodic boundary conditions on [-1,1]×[-1,1]
    - 6 parameter regimes (gliders, bubbles, maze, worms, spirals, spots)
    - Precision-sensitive stiff PDE; 1001 steps → long rollout
    - CNextU-net best LR: 1e-4 (paper Table 6)
"""
...
BATCH_SIZE = 256  # configs/data/gray_scott_reaction_diffusion.yaml
NUM_WORKERS = 12
GRAD_CLIP = 1.0
...
def get_base_config(
    *,
    ...
    Args:
        batch_size: Per-GPU batch size (default from paper: 256).
        learning_rate: Peak learning rate (CNextU-net best: 1e-4).
