...
"""Shared config for acoustic_scattering_maze experiments (v2).

Dataset: acoustic_scattering_maze
    - 2D, 256×256, 3 dynamic fields, 2 constant fields
    - Dynamic fields: pressure, velocity_{x,y}
    - Constant fields: density, speed_of_sound (maze geometry)
    - 1600 train trajectories × 202 timesteps
    - CNextU-net best LR: 1e-3 (paper Table 6)
"""
...
BOUNDARY_CONDITIONS = {"x": ("WALL", "OPEN"), "y": ("WALL", "OPEN")}

N_FIELDS = 3  # pressure, velocity_{x,y}
N_CONSTANT_FIELDS = 2  # density, speed_of_sound
IN_CHANNELS = N_STEPS_INPUT * N_FIELDS + N_CONSTANT_FIELDS  # 14
OUT_CHANNELS = N_FIELDS  # 3

# 16 files × 100 trajs = 1600 trajs, 202 timesteps each → 198 windows
# Total: 1600 × 198 = 316,800 samples
SAMPLES_PER_EPOCH = 316_800

# ─── Training constants (shared across models) ───────────────────────────────
TRAINING_ITERATIONS = 110_000
WARMUP_ITERATIONS_PERCENTAGE = 0.05

BATCH_SIZE = 64  # configs/data/acoustic_scattering_maze.yaml
LEARNING_RATE = 1e-3
WEIGHT_DECAY = 1e-4
