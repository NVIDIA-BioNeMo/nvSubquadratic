...
if [ -z "$1" ]; then
    echo "Usage: sbatch [--job-name=NAME] examples/well/submit_well_ivi_1gpu_geodude.sh <config.py> [extra args...]"
    echo "  e.g. sbatch --job-name=well-hyena examples/well/submit_well_ivi_1gpu_geodude.sh examples/well/MHD_64/cfg_hyena.py"
    exit 1
fi
