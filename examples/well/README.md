...
2. **Lightning Wrapper** (`experiments/lightning_wrappers/well_lightning_wrapper.py`): Handles input formatting (flattening timesteps into channels), single-step training, autoregressive rollout for validation/test, and metric computation on denormalized (physical-scale) data.

3. **Network**: Any `ResidualNetwork` architecture from nvSubquadratic (Hyena, Attention, CKConv). Input/output channels are set automatically from the dataset metadata.
...
2. Update these parameters in the config file:
...
3. Download the data: `bash scripts/download_well.sh <new_dataset>`
4. Run: `PYTHONPATH=. python experiments/run.py --config examples/well/<new_dataset>/cfg_hyena.py`
