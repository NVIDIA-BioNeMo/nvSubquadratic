"""Hyena ResNet baseline for ARC-AGI.

Same as cfg_hyena_rearc.py but using the custom subquadratic_ops_torch CUDA FFT
kernel (fft_backend='subq_ops') instead of torch.fft. Used for benchmarking the
custom kernel against the torch.fft baseline.
"""
...
def get_config() -> ExperimentConfig:
    ...
    # compile_compatible_fftconv not needed: subq_ops backend uses a custom CUDA
    # kernel (not torch.fft), so there is no complex64 Inductor issue.
