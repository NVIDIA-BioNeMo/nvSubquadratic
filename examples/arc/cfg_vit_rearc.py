...
MLP_DIM = 512
PATCH_SIZE = 2
MAX_SIZE = 32  # 32×32 canvas → seq length 256 at patch_size=2 (matches cfg_vit.py)
...
def get_config():
    """ARCViT baseline trained on ARC + RE-ARC, matching reference setup.

    Key matches vs varc reference:
    - batch_size = 32 per GPU (global 64)
    - val on training tasks' test queries (in-distribution, trained task tokens)
    - same exact-match metric definition
    """
    ...
    config.dataset = LazyConfig(ARCDataModule)(
        ...
        # Validate on training tasks' test queries (same as reference eval_split="training", eval_subset="test")
    ...
    config.net = LazyConfig(ARCViT)(
        num_tasks=400,  # Reference only assigns task tokens to the 400 training tasks
        embed_dim=EMBED_DIM,
        depth=DEPTH,
        num_heads=NUM_HEADS,
        mlp_dim=MLP_DIM,
