"""Hyena ResNet for ARC-AGI — ViT5-style task-token-in-sequence conditioning.

Identical to cfg_hyena_rearc_subq_ops.py in all architectural choices, but changes
the task injection mechanism from 'broadcast' (one-shot add at the input embedding)
to 'seq_concat' (task token prepended as an extra spatial row in the patch grid).

Conditioning mechanism comparison
──────────────────────────────────
  broadcast  (baseline): task_tok added once to x before the ResNet → signal
             fades as residual blocks accumulate activations. Zero ongoing influence.

  seq_concat (this config): task_tok prepended as row 0 of the 16×16 patch grid
             (making it 17×16) before the first residual block. The 3×3 short-conv
             in each Hyena layer then lets the top row of spatial patches "see" the
             task token at every depth.  After all blocks, the task row is stripped
             before Unpatchify.

  Analogy to ViT: ARCViT prepends the task token to the 1D patch sequence so every
  patch attends to it at every layer via self-attention. seq_concat is the closest
  ResNet analog — task token participation through local receptive field rather than
  full attention.

Spatial dimensions inside the blocks: [B, 17, 16, EMBED_DIM] (17 = 16 patches + 1
task row). The global Hyena FFT conv is applied over the full 17×16 grid, so the task
row's signal propagates globally in addition to locally via the short-conv.

Run on 2× geodude GPUs with batch_size=128×2=256 effective global BS.
"""
...
def get_config() -> ExperimentConfig:
    """Hyena ResNet ARC ablation: ViT5-style task token in sequence (seq_concat)."""
    ...
    config.net = LazyConfig(ARCResNet)(
        ...
        task_injection="seq_concat",  # ViT5-style: task token prepended as row in patch grid
    )

    config.wandb = WandbConfig(entity="implicit-long-convs", project="nvsubquadratic", job_group="arc_seq_concat")
