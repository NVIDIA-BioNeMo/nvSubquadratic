...
| Hyena (per-layer broadcast)       | 19.04M | 256 (128×2)       | 2x A100    | 🔄 Running (SLURM 21906660) | *n/a*                  | Config: `conditioning_ablation/cfg_hyena_rearc_per_layer_broadcast_subq_ops.py` · re-inject task token additively before each residual block via `AdditiveCondResidualBlock(cond_gate="none")` · zero extra params · `task_injection="per_layer_broadcast"` on ARCResNet                                                                         |
| Hyena (per-layer broadcast gated) | 19.04M | 256 (128×2)       | 2x A100    | 🔄 Running (SLURM 21906661) | *n/a*                  | Config: `conditioning_ablation/cfg_hyena_rearc_per_layer_broadcast_gated_subq_ops.py` · same as above but `cond_gate="scalar_zero_init"` → 12 extra scalar params (one tanh gate per block, zero-init) · starts identical to unconditional ResNet at step 0 · isolates the effect of the gate vs the ungated variant                             |
