...
| Hyena (patch=1)                   | 13.25M | 256 (64×1×accum4) | 1x a6000   | 🔄 Running (SLURM 155329)   | **60.7%** (epoch 52)   | Config: `cfg_hyena_rearc_subq_ops_patch1.py` · [WandB lac5r48z](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/lac5r48z) · pixel-space seq_len=1024 · Resumed from `epoch=52-step=85489.ckpt` (wall-time cancel of 154951) · resuming from epoch 54                                                                                    |
| ARCViT AdaLN                      | 32.96M | 256 (128×2)       | 2x geodude | ⏳ Not started              | *n/a*                  | Config: `conditioning_ablation/cfg_vit_rearc_adaln.py` · DiT-style AdaLN-Zero on ViT · task token NOT prepended (removed from sequence) · **expected to stagger**: ViT's native strength is task token participating in every attention layer; AdaLN replaces that with a weaker side-channel → removing sequence participation likely hurts ViT |
...
| Hyena (multi-layer broadcast)     | ~19M   | 256 (128×2)       | 2x geodude | ⏳ Not started              | *n/a*                  | Config: `cfg_hyena_rearc_multibroadcast.py` (to create) · re-inject task token additively before each residual block · zero extra params · cheap sanity check for residual-stream conditioning vs input-only broadcast                                                                                                                           |
