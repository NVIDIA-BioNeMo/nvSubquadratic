...
| Hyena (grid=single + circular)        | ~19M   | 256 (128×2)       | 2x geodude | 🔄 Running (SLURM 155589)   | *n/a*                  | Config: `cfg_hyena_rearc_subq_ops_circular.py` · `fft_padding="circular"`, `fft_backend="torch_fft"`, `compile=False` (subq_ops asserts against circular; Inductor can't handle complex64 in circular FFT with compile=True; 155567/155578 failed for these reasons) · ablation: does periodic padding help with edge-task generalisation?                                                                                             |
...
| Hyena (seq_concat / task-in-sequence) | ~19M   | 256 (128×2)       | 2x geodude | 🔄 Running (SLURM 155568)   | *n/a*                  | Config: `cfg_hyena_rearc_seqtoken_subq_ops.py` · `task_injection="seq_concat"`: task_tok prepended as row 0 of 16×16 patch grid (→ 17×16) before all 12 blocks; stripped before Unpatchify · ViT5-style: task participates in short-conv receptive field and global FFT conv at every depth · new `ARCResNet.forward()` branch                                                                                                         |
