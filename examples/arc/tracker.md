...
> - **Exception:** `fft_backend="subq_ops"` only supports `fft_padding="zero"`. If you need `fft_padding="circular"` or `"causal"`, you must use `fft_backend="torch_fft"` instead.
...
### 6. Canvas representation: 32×32 discrete tokens (ours) vs 64×64 pixel image (VARC)

**VARC:** Renders each ARC grid onto a 64×64 pixel canvas as an RGB image (one ARC cell = one pixel at scale 1).
The ViT processes 4×4 pixel patches → 16×16 = 256 patch tokens from a real image.

**Ours:** Maps each ARC grid onto a 32×32 **integer canvas** (one ARC cell = one discrete color index).
These are fed through a **learned 12-color embedding** table → 2×2 patch projection → 16×16 = 256 patch tokens.

Both produce the same sequence length (256 tokens), but the input modality is fundamentally different.

**Why ours may be better:** Color identity is categorical — an embedding table is more principled than learning
to decode ARC colors from RGB pixel values. This also avoids any color-rendering bias from the VARC codebase.
Our 83.8% val exact match (vs VARC reference ~73%) is consistent with this being at least as good.

**Side effects:**

- Resolution augmentation is integer zoom (scale=1..N, each cell → N×N tokens) vs sub-pixel rendering in VARC.
- `num_colors=12` in ours (10 ARC colors + IGNORE_INDEX + PAD_INDEX) vs `num_colors=10` in VARC (relies on silent
  CUDA UB for OOB embedding lookups — see divergence #2 above).
- **TTT callback was broken** (`network.task_token_embed` doesn't exist; actual path is
  `network.embedding.task_embed`). No `val_ttt/` metrics have ever been logged.
  Fixed in `scripts/evaluation/eval_arc_ttt.py`.

**TTT evaluation runs:**

| Model           | Config                               | Checkpoint                   | SLURM  | Status                      | TTT exact_match |
| :-------------- | :----------------------------------- | :--------------------------- | :----- | :-------------------------- | :-------------- |
| ARCViT (best)   | `cfg_vit_rearc.py`                   | `epoch=469-step=757640.ckpt` | 155966 | ❌ Broken (task-token only) | ~1%             |
| Hyena (patch=1) | `cfg_hyena_rearc_subq_ops_patch1.py` | `epoch=131-step=212916.ckpt` | 155967 | ❌ Broken (task-token only) | ~1%             |
| ARCViT (best)   | `cfg_vit_rearc.py`                   | `epoch=469-step=757640.ckpt` | 156024 | 🔄 Running                  | *Pending*       |
| Hyena (patch=1) | `cfg_hyena_rearc_subq_ops_patch1.py` | `epoch=131-step=212916.ckpt` | 156025 | 🔄 Running                  | *Pending*       |
...
| ARCViT                                | 17.20M | 256 (128×2)       | 2x geodude | ❌ Failed (SLURM 154955)      | **83.8%** (epoch 469)  | Config: `cfg_vit_rearc.py` · [WandB rzdmi235](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/rzdmi235) · Continuation of 153872 · resumed from `epoch=190-step=307892.ckpt` · best ckpt `epoch=469-step=757640.ckpt` · failed at epoch ~500 (OOM, exit -15/-6)                                                                                                                                                                                                                     |
| Hyena AdaLN (stable)                  | 24.35M | 256 (128×2)       | 2x geodude | ❌ Timeout (SLURM 154949)     | **71.0%** (epoch 346)  | Config: `conditioning_ablation/cfg_hyena_rearc_adaln_stable_subq_ops.py` · [WandB dxnvk65v](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/dxnvk65v) · Stabilised: tanh gate + condition_proj WD=1e-4 · best ckpt `epoch=346-step=559364.ckpt` · hit wall-time at epoch 455 (last val 69.3%)                                                                                                                                                                                       |
| Hyena (grid=double)                   | 13.70M | 256 (256×1)       | 1x a6000   | ❌ Cancelled (SLURM 154950)   | **68.4%** (epoch 368)  | Config: `cfg_hyena_rearc_subq_ops_grid_double.py` · [WandB ts8jamnr](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/ts8jamnr) · grid_type='double': 31×31 SIREN kernel vs 15×15 baseline · best ckpt `epoch=368-step=594828.ckpt` · cancelled (KeyboardInterrupt) at epoch ~370                                                                                                                                                                                                    |
| Hyena (patch=1)                       | 13.25M | 256 (64×1×accum4) | 1x a6000   | 🔄 Running (SLURM 156027)     | **60.7%** (epoch 131)  | Config: `cfg_hyena_rearc_subq_ops_patch1.py` · [WandB lac5r48z](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/lac5r48z) · pixel-space seq_len=1024 · Resumed from `epoch=131-step=212916.ckpt` (wall-time cancel of 21874746) · autoresume enabled · 3-day wall-time                                                                                                                                                                                                              |
...
| Hyena (grid=single + circular)        | ~19M   | 256 (128×2)       | 2x geodude | 🔄 Running (SLURM 156026)     | **66.1%** (epoch 260)  | Config: `cfg_hyena_rearc_subq_ops_circular.py` · [WandB lxphnb5l](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/lxphnb5l) · `fft_padding="circular"`, `fft_backend="torch_fft"` · best ckpt `epoch=260-step=420732.ckpt` · Resumed from wall-time cancel of 155589 · autoresume enabled · 3-day wall-time                                                                                                                                                                         |
| Hyena (per-layer broadcast)           | 19.04M | 256 (128×2)       | 2x H100    | ❌ Cancelled (SLURM 21931841) | **9.84%** (ep ~14097)  | Config deleted · `AdditiveCondResidualBlock(cond_gate="none")` · catastrophically bad result: task_tok accumulated 12× in residual stream, dominated features · no bug in code, design is fundamentally flawed · **code reverted at commit `535e814`**; `AdditiveCondResidualBlock` and `per_layer_broadcast` path removed from codebase                                                                                                                                                     |
| Hyena (per-layer broadcast gated)     | 19.04M | 256 (128×2)       | 2x H100    | ❌ Cancelled (SLURM 21931842) | **8.83%** (ep ~14057)  | Config deleted · `AdditiveCondResidualBlock(cond_gate="scalar_zero_init")` · same failure mode as ungated; even gated version converged to near-random · **code reverted at commit `535e814`**                                                                                                                                                                                                                                                                                               |
| Hyena (seq_concat / task-in-sequence) | ~19M   | 256 (128×2)       | 2x geodude | 🔄 Running (SLURM 155568)     | **64.8%** (epoch 323)  | Config: `cfg_hyena_rearc_seqtoken_subq_ops.py` · [WandB y4ow1gie](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/y4ow1gie) · `task_injection="seq_concat"`: task_tok prepended as row 0 of 16×16 patch grid (→ 17×16) before all 12 blocks; stripped before Unpatchify · best ckpt `epoch=323-step=522288.ckpt` · currently epoch ~337, val=64.4% · **underperforming vs AdaLN (73.0%)** — candidate for cancellation; plateau visible, further training unlikely to close the gap |
| Hyena (patch=1 + circular + AdaLN)    | ~13M   | 256 (64×1×accum4) | 1x a6000   | 🔄 Running (SLURM 156029)     | *Pending*              | Config: `cfg_hyena_rearc_subq_ops_patch1_circular_adaln.py` · New run · Combines patch_size=1 (seq_len=1024) + `fft_padding="circular"` + `AdaLNZeroResidualBlock` · `fft_backend="torch_fft"` (circular requires it) · hypothesis: additive improvements from all three ablation winners                                                                                                                                                                                                    |
