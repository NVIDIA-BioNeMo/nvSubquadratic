"""Hyena ResNet baseline for ARC-AGI, matching the reference ViT training setup.

Data, batch size, LR, val split, and precision are identical to cfg_vit_rearc.py.
The backbone is a ResidualNetwork with Hyena (QKV + CKConv global conv) mixers.

Key design choices vs ARCViT:
- Task token is broadcast-added to the colour embedding at every spatial position
  rather than prepended as a sequence element.
- No attention mask: Hyena naturally handles zero-padded borders via its convolution.
- fft_padding="zeros": ARC grids are non-periodic.
- patch_size=2 → 32×32 patched resolution (same as ViT, seq_len=1024).
"""
...
def get_config() -> ExperimentConfig:
    """Hyena ResNet trained on ARC + RE-ARC, matching the reference ViT setup."""
