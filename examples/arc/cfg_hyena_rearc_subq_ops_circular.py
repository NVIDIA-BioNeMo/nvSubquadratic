"""Hyena ResNet for ARC-AGI — circular FFT padding ablation.

Identical to cfg_hyena_rearc_subq_ops.py except fft_padding is changed from
'zero' to 'circular' and fft_backend switched to 'torch_fft' (subq_ops only
supports zero-padded convolutions).

fft_padding controls the boundary assumption of the global Hyena convolution:
  - 'zero'     (baseline): zero-pad signal before FFT → treats spatial boundaries
                as non-periodic. Correct assumption for ARC grids, which have hard
                edges and are placed on a zero/pad-colour canvas.
  - 'circular' (this config): circular/periodic FFT → treats the 16×16 patch grid
                as a torus. Wrong in a strict sense for ARC, but may help the model
                learn longer-range associations by wrapping spatial context across
                the canvas edges.

Motivation: circular convolutions are the default in most FFT-conv literature and
may bring a free accuracy boost if boundary effects are not critical for ARC tasks.
The short-conv (3×3, padding=1) is already effectively circular at that scale.
...
FFT_PADDING = "circular"  # periodic boundary (ablation vs 'zero' baseline)
OMEGA_0 = 30.0
DROPOUT = 0.1
...
def get_config() -> ExperimentConfig:
    """Hyena ResNet ARC ablation: circular FFT padding (vs zero-padding baseline)."""
    ...
    config.compile = False  # torch.compile + Inductor cannot handle complex64 in circular FFT functions
            ...
            sequence_mixer_cfg=LazyConfig(QKVSequenceMixer)(
                ...
                mixer_cfg=LazyConfig(Hyena)(
                    global_conv_cfg=LazyConfig(CKConvND)(
                        ...
                        use_fp16_fft=False,
                        fft_backend="torch_fft",  # subq_ops only supports zero-padding
    ...
    config.wandb = WandbConfig(
        entity="implicit-long-convs", project="nvsubquadratic", job_group="arc_padding_ablation"
    )
