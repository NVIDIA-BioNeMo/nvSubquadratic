"""Hyena ResNet + per-layer additive task-token conditioning (zero-init scalar gate).

Clones the broadcast baseline (examples/arc/cfg_hyena_rearc_subq_ops.py) but swaps
the one-shot task-token add for a gated re-broadcast at the input of every residual
block:

  before each block:  x <- x + tanh(alpha_l) * task_tok[:, None, None, :]

alpha_l is a zero-initialised scalar parameter per block (12 extra params total for
NUM_BLOCKS=12). At step 0 the additive term vanishes so training starts identical to
the unconditional Hyena ResNet; the gate opens gradually — same philosophy as AdaLN-Zero
but ~6x fewer conditioning parameters. Reference configs:
  ...
  - ungated variant    : examples/arc/conditioning_ablation/cfg_hyena_rearc_per_layer_broadcast_subq_ops.py
"""
...
# ── Conditioning ──────────────────────────────────────────────────────────────
COND_GATE = "scalar_zero_init"  # one learned scalar per block, zero-initialised tanh gate
...
def get_config() -> ExperimentConfig:
    ...
    config.wandb = WandbConfig(
        entity="implicit-long-convs", project="nvsubquadratic", job_group="arc_per_layer_broadcast_gated"
    )
