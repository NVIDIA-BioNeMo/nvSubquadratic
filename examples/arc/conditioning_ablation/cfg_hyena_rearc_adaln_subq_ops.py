"""Hyena ResNet + AdaLN-Zero conditioning for ARC-AGI.

Replaces the plain broadcast baseline with DiT-style AdaLN-Zero modulation:
...
The projection head is zero-initialised, so training starts as a plain unconditional
Hyena ResNet and the conditioning gradually switches on — same gradient landscape as the
broadcast baseline at step 0.

Part of the conditioning ablation study. Reference configs:
  - broadcast baseline : examples/arc/cfg_hyena_rearc_subq_ops.py
  - FiLM on SIREN      : examples/arc/cfg_hyena_rearc_film_subq_ops.py
"""
...
def get_config() -> ExperimentConfig:
    """Hyena ResNet trained on ARC + RE-ARC with AdaLN-Zero task-token conditioning."""
    ...
    config.net = LazyConfig(ARCResNet)(
        ...
        task_injection="film",  # routes task_tok as condition tensor into every block
    )
