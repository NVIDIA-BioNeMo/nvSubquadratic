"""Hyena ResNet + stabilised AdaLN-Zero conditioning for ARC-AGI.

Identical to cfg_hyena_rearc_adaln_subq_ops.py but with three changes that
address the late-training divergence observed in the original AdaLN run:

  1. Tanh gate  — gates are bounded to (-1, 1) via torch.tanh, preventing
                  unbounded residual growth across the 12 stacked blocks.
                  Applied in AdaLNZeroResidualBlock.forward (residual_block.py).
                  Zero-init is preserved: tanh(0) = 0.

  2. condition_proj weight decay — the condition_proj Linear receives
                  weight_decay=1e-4 via the _weight_decay param attribute,
                  regularising the conditioning path. Applied automatically
                  in AdaLNZeroResidualBlock.__init__ (residual_block.py).

  3. Condition dropout (p=0.1) — during training, 10 % of samples have their
                  task token zeroed out, forcing the model to stay functional
                  without conditioning and preventing over-reliance on a
                  potentially noisy condition signal. Set via cond_dropout_prob
                  in ARCResNet.

Part of the conditioning ablation study.
"""
...
# ── Conditioning stabilisation ────────────────────────────────────────────────
COND_DROPOUT_PROB = 0.1  # fraction of samples where task token is zeroed during training
...
def get_config() -> ExperimentConfig:
    """Stabilised Hyena ResNet with AdaLN-Zero conditioning (tanh gate + WD + cond dropout)."""
    ...
    config.wandb = WandbConfig(entity="implicit-long-convs", project="nvsubquadratic", job_group="arc_adaln_stable")
