...
#SBATCH --gpus=2
#SBATCH --job-name=arc_vit_p1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=2-00:00:00
#SBATCH --mem=64G
#SBATCH --output=logs/arc_p1_%A.out
...
if [ -z "$1" ]; then
    echo "Usage: sbatch [--job-name=NAME] examples/arc/submit_arc_2gpu_a100.sh <config.py> [extra args...]"
    echo "  e.g. sbatch examples/arc/submit_arc_2gpu_a100.sh examples/arc/cfg_vit_rearc_patch1.py"
    exit 1
fi
