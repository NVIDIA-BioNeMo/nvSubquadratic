...
(64x64 → 4096 elements).  Each segment is flanked by short coloured markers.
The model must output the target digit coloured to match its markers.

Unlike simple_copy_1d (causal), this task requires the model to attend to
the full sequence (random placement + multiple items), so all models are
...
INPUT_CHANNELS = 3  # RGB with coloured boundary markers
OUTPUT_CHANNELS = 3  # RGB digit in marker colour
NUM_ITEMS = 4
...
def base_experiment_config(
    ...
    config.wandb = WandbConfig(
        ...
        job_group="spatial_recall_v2_1d_color_conditioning",
    )
