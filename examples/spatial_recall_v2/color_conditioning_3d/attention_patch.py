...
"""3D Color Conditioning -- Attention XS with Patchify (v2).

Hidden dim: 240, 4 blocks, num_heads=8, head_dim=30.
Patch_size=4 (8x64x64 -> 2x16x16 = 512 tokens).
3D RoPE enabled (head_dim=30, 30 % 6 == 0).

Attention cost drops from O(32768^2) to O(512^2).
...
PATCH_SIZE = 4  # 8/4 x 64/4 x 64/4 = 2x16x16 = 512 tokens
