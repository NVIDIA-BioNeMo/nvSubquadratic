...
"""CCNN diffusion on ImageNet 128x128, JiT-B matched (768 hidden, 12 blocks, patch 8)."""
...
# Dataset ----------------------------------------------------------------------
BATCH_SIZE = 128  # Per GPU — halved vs 64px to fit memory
NUM_WORKERS = min(12, os.cpu_count() - 2 or 4)
FINAL_IMAGE_SIZE = 128
PATCH_SIZE = 8  # 128 / 8 = 16x16 tokens (same count as 64px / 4)
...
WEIGHT_DECAY = 0.0
GRAD_CLIP = 1.0
ACCUMULATE_GRAD_STEPS = 2  # 128 * 4 GPUs * 2 = 1024 effective batch size
...
def get_config() -> DiffusionExperimentConfig:
    ...
    # CCNN Model — JiT-B matched at 128x128
    ...
    config.wandb = WandbConfig(
        job_group="imagenet_diffusion_ccnn_128",
        entity=WANDB_ENTITY,
        tags=["ccnn", "diffusion", "imagenet128", "jit-B-matched"],
    )
