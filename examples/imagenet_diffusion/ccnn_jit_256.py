...
"""CCNN diffusion on ImageNet 256x256, JiT-B matched (768 hidden, 12 blocks, patch 16)."""
...
# Dataset ----------------------------------------------------------------------
BATCH_SIZE = 64  # Per GPU — reduced for 256px memory
NUM_WORKERS = min(12, os.cpu_count() - 2 or 4)
FINAL_IMAGE_SIZE = 256
PATCH_SIZE = 16  # 256 / 16 = 16x16 tokens (same count as 64px / 4)
...
WEIGHT_DECAY = 0.0
GRAD_CLIP = 1.0
ACCUMULATE_GRAD_STEPS = 4  # 64 * 4 GPUs * 4 = 1024 effective batch size
...
def get_config() -> DiffusionExperimentConfig:
    ...
    # CCNN Model — JiT-B matched at 256x256
    ...
    config.wandb = WandbConfig(
        job_group="imagenet_diffusion_ccnn_256",
        entity=WANDB_ENTITY,
        tags=["ccnn", "diffusion", "imagenet256", "jit-B-matched"],
    )
