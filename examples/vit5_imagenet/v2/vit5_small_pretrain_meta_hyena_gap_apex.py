"""ViT-5-Small + Hyena + GAP + MetaSIRENKernelND — Apex FusedLAMB variant.

Same as vit5_small_pretrain_hyena_gap_apex.py except a single centralized
MetaSIRENKernelND generates all 12 per-layer convolutional kernels in one forward
pass (shared SIREN positional embedding + hidden backbone, single output
linear projecting to num_layers * hidden_dim).  Each block's CKConvND has
kernel_cfg=None and mask_cfg=None — it receives the precomputed (already-
masked) kernel from the network level.
"""
...
from nvsubquadratic.modules.meta_kernels_nd import MetaSIRENKernelND
...
# TODO(@dwromero) -> adjust to shared SIREN hyperparameters so params roughly match!
...
def get_config() -> ExperimentConfig:
    """Return the ViT-5-Small + Hyena (GAP) + MetaSIRENKernelND config."""
    ...
    # ─── Network ────────────────────────────────────────────────────────────
    # CKConvND with kernel_cfg=None and mask_cfg=None (kernels from MetaSIRENKernelND)
    ...
    # Centralized kernel generator: single SIREN for all layers
    meta_kernel_cfg = LazyConfig(MetaSIRENKernelND)(
        num_layers=NUM_BLOCKS,
        out_dim=HIDDEN_DIM,
        data_dim=2,
        mlp_hidden_dim=KERNEL_MLP_HIDDEN_DIM,
        num_mlp_layers=KERNEL_NUM_LAYERS,
    ...
    config.net = LazyConfig(ViT5ClassificationNet)(
        ...
        meta_kernel_cfg=meta_kernel_cfg,
        meta_kernel_grid_type="double",
    )
