"""ViT-5-Small + Hyena ImageNet-1k — CLS-row, gated attention variant.

Based on v2/vit5_small_pretrain_hyena_cls_row_apex.py with a gated
attention-style dual nonlinearity following Hua et al. (2022):
- First gate: SiLU — z = Q * SiLU(K)
- Second gate: Sigmoid — y = conv_out * Sigmoid(V)

CLS-row architecture: CLS + 13 registers as extra row → 15×14 grid.
All other settings (output RMSNorm, L2 QK-norm, DALI fused) match the
standard v2 CLS-row config.
"""
