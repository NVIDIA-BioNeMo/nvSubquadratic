"""ViT-5-Small + Hyena ImageNet-1k — CLS-row, gated attention + FiLM-conditioned SIREN kernels.

Based on vit5_small_pretrain_hyena_cls_row_apex_gated.py with input-dependent
SIREN kernels via FiLM conditioning from register tokens.

Each ViT5ResidualBlock extracts register tokens from the normalized input,
pools them via a learnable weighted average (RegisterPooling), and passes the
conditioning vector through to the SIREN kernel generator where it modulates
every hidden layer via FiLM (gamma * h + beta).
...
NUM_REGISTERS = NUM_PATCHES_W - 1  # 13 — fills the extra row: [CLS, regs, patches] -> (H'+1)xW' grid
...
def get_config() -> ExperimentConfig:
    """Return the ViT-5-Small + Hyena CLS-row config with FiLM-conditioned SIREN kernels."""
    ...
    # ─── Network ────────────────────────────────────────────────────────────
    # FiLM generator config for SIREN kernels.
    # num_film_layers = KERNEL_NUM_LAYERS - 1 (all hidden layers, excluding output linear).
    ...
    # RegisterPooling config — one instance per block (layer-wise weights).
