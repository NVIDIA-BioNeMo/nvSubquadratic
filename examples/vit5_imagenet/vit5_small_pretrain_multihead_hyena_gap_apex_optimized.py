"""ViT-5-Small + Multi-Head Hyena + GAP ImageNet-1k — Optimized mixer variant.

Based on vit5_small_pretrain_multihead_hyena_gap_apex_qk_rmsnorm.py with the
same three mixer-level changes applied in vit5_small_pretrain_hyena_gap_apex_optimized.py:

1. SiLU on the value gate: adds a nonlinearity to the otherwise fully-bilinear
   mixer (y = conv_out * SiLU(v) instead of y = conv_out * v).
2. QK norm effectively q-only: PerHeadRMSNorm is still configured, but k_norm
   is skipped at runtime because gate_nonlinear is non-Identity.
3. Output RMSNorm added: stabilizes the mixer output before it enters the
   residual stream (Mamba2-style).

Key differences from the base multi-head config:
- CKConvMultiheadND with dense within-head mixing (6 heads, 64 head_dim).
- SIREN kernel out_dim = NUM_HEADS * HEAD_DIM * HEAD_DIM (dense kernel per head).
- SiLU gate, q-only PerHeadRMSNorm, output RMSNorm.
"""
...
def get_config() -> ExperimentConfig:
    """Return the ViT-5-Small + Multi-Head Hyena (GAP, optimized) config with Apex FusedLAMB."""
