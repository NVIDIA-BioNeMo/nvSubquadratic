"""Full Hyena – 12 Hyena blocks, block-diagonal learnable-ω₀ kernel.
...
``BlockAlignedGaussianModulationND``.  Combines two ideas:

1. **Block-diagonal MLP init + per-block ω₀ schedule** — same defaults
   as ``full_hyena_blockdiag`` (``num_blocks=8``, linear ω₀ schedule in
...
2. **Learnable per-row ω₀ scale** — the ``2π·ω₀`` factor is pulled out
   ...
   a learnable per-row scale clamped to ``[0, 2]`` (so each row can
   double its effective ω₀ during training).
...
def get_config() -> ExperimentConfig:
    """Build the all-Hyena config with block-diagonal learnable-ω₀ kernels."""
