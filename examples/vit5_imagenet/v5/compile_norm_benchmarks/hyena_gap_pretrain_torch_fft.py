...
"""ViT-5-Small Hyena GAP gated — torch_fft backend (baseline for cf_norm comparison).

Identical to hyena_gap_pretrain.py except:
- ``fft_backend="torch_fft"`` instead of ``"subq_ops"``
- ``compile_compatible_fftconv=True`` so torch.compile can trace through the FFT

This is the channel-last control for ``hyena_gap_pretrain_cf_norm.py`` (torch_fft +
channel-first norms).  Comparing the two isolates the effect of channel-first norms
when the FFT conv is fully visible to the compiler.
"""
...
def get_config() -> ExperimentConfig:
    """Build ViT-5-Small Hyena GAP gated config with torch_fft backend."""
