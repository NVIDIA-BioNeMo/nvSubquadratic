"""ViT-5-Small + Hyena ImageNet-1k — CLS-row, gated, wider FiLM-conditioned SIREN, RoPE.

Same as ``vit5_small_pretrain_hyena_cls_row_gated_film_rope.py`` but with a wider
SIREN kernel network and FiLM bottleneck for increased kernel expressivity:
- SIREN hidden dim: 32 -> 64
- SIREN layers: 3 -> 4  (one more FiLM-conditioned hidden layer)
- FiLM hidden dim: 64 -> 128

This adds ~600K parameters total (~3% of 22M model) while significantly increasing
the kernel's function approximation capacity and the FiLM conditioning bandwidth.
...
# ─── Hyena / SIREN kernel hyperparameters (wider) ───────────────────────────────
KERNEL_MLP_HIDDEN_DIM = 64   # doubled from 32
KERNEL_NUM_LAYERS = 4         # increased from 3
KERNEL_EMBEDDING_DIM = 32
KERNEL_OMEGA_0 = 10.0
KERNEL_HIDDEN_OMEGA_0 = 1.0

# ─── FiLM conditioning (wider) ──────────────────────────────────────────────────
FILM_HIDDEN_DIM = 128  # doubled from 64
...
def get_config() -> ExperimentConfig:
    """Return the ViT-5-Small + Hyena CLS-row gated + wider FiLM + RoPE config."""
    ...
    # ─── FiLM generator (wider) ──────────────────────────────────────────
    film_cfg = LazyConfig(KernelFiLMGenerator)(
        cond_dim=HIDDEN_DIM,
        kernel_hidden_dim=KERNEL_MLP_HIDDEN_DIM,
        num_film_layers=KERNEL_NUM_LAYERS - 1,  # 3 FiLM-conditioned hidden layers
