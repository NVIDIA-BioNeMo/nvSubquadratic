"""ViT-5-Small + Hyena ImageNet-1k — distributed registers + cross-attention + FiLM.

Evenly distributes register tokens among patches (Mamba-R, arXiv:2405.14858).
Registers are stripped before Hyena's 2D convolution and updated via lightweight
cross-attention (registers attend to patches).

- CKConvND (depthwise) on pure 14x14 patch grid.
- FiLM-conditioned SIREN kernels (input-dependent via register pooling).
- Register cross-attention: 1-head, registers (Q) attend to patches (K, V).
...
from nvsubquadratic.modules.register_patch_comm import RegisterCrossAttention
...
def get_config() -> ExperimentConfig:
    """Return the ViT-5-Small + distributed registers + cross-attention + FiLM config."""
    ...
    config.net, trainer_overrides = build_distributed_reg_network(
        ...
        register_comm_cfg=LazyConfig(RegisterCrossAttention)(
            hidden_dim=HIDDEN_DIM,
            num_heads=1,
            qk_norm=True,
        ),
