"""ViT-5-Small + Hyena ImageNet-1k — CLS-row, gated, wider FiLM, RoPE, GRN.

Combines the wider FiLM + RoPE config with Global Response Normalization (GRN)
from ConvNeXt V2 (Woo et al., 2023):
- SIREN hidden dim: 64 (wider than base 32)
- SIREN layers: 4 (vs base 3)
- FiLM bottleneck dim: 128 (wider than base 64)
- GRN after mixer output for inter-channel competition
- EMA (decay=0.99996) with val/acc_ema checkpoint monitoring
...
def get_config() -> ExperimentConfig:
    """Return the ViT-5-Small + Hyena CLS-row gated + wider FiLM + RoPE + GRN + EMA config."""
