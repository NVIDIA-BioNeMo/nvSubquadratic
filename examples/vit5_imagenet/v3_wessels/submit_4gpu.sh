...
if [ -z "$1" ]; then
    echo "Usage: sbatch [--job-name=NAME] examples/vit5_imagenet/v3/submit_4gpu.sh <config.py> [extra args...]"
    echo "  e.g. sbatch --job-name=v3-gated examples/vit5_imagenet/v3/submit_4gpu.sh examples/vit5_imagenet/v3/vit5_small_pretrain_hyena_cls_row_apex_gated.py"
    exit 1
fi
