"""ViT-5-Small + Multi-Head Hyena ImageNet-1k — register reduction head (Mamba-R).

Drops the CLS token and uses 14 registers (filling the entire first row) with
a register reduction head for classification (arXiv:2405.14858):
...
def get_config() -> ExperimentConfig:
    """Return the ViT-5-Small + Multi-Head Hyena register-recycling config."""
    config = get_base_config()

    mixer_cfg = build_multihead_hyena_mixer(film_cfg=None, use_rope=False)
