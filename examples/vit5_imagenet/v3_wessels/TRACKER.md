...
> Last updated: 2026-03-18. See W&B for live curves.
...
| `…grouped_hyena_cls_row_gated_film_ema` | Grouped (6g) | No | 32h/64d | No | CLS | [qmvsolpk](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/qmvsolpk) | 0.683 (ep118) | Running (20842258) |
...
| `…multihead_hyena_cls_row_gated_film_ema` | MH (6h, d=64) | No | 32h/64d | No | CLS | [ladnbcd9](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/ladnbcd9) | 0.005 (ep8) | Running (20807577) |
| `…distributed_reg_crossattn_film_ema` | Depthwise | No | 32h/64d | No | DistReg14+XAttn | [vcmnt9ey](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/vcmnt9ey) | — (ep0) | Running (20880555) |
| `…distributed_reg_localpool_film_ema` | Depthwise | No | 32h/64d | No | DistReg14+Pool | [406qsc1o](https://wandb.ai/implicit-long-convs/nvsubquadratic/runs/406qsc1o) | — (ep0) | Running (20880577) |
...
- **Multi-head + FiLM** resubmitted as 20807577; very early (ep8, 0.5% acc). Also tried as 20784221.
- **Grouped conv (6 groups) + FiLM** running as 20842258; at ep118, val/acc_ema=0.683 — tracking well.
...
- **Distributed registers (Mamba-R)**: Two new experiments launched — registers are evenly interleaved among patches, stripped before Hyena's 2D conv, and updated via cross-attention (20880555) or local pooling (20880577). Motivated by Mamba-R (arXiv:2405.14858) which showed +0.9% from distributing vs prepending registers.
...
- [x] Launch distributed register experiments (Mamba-R style) — running as 20880555 (cross-attn) and 20880577 (local pool)
...
- [ ] Monitor grouped conv run (20842258) — early results look promising
- [ ] Monitor distributed register runs — compare cross-attn vs local pool vs prepend baseline (0.8131)
