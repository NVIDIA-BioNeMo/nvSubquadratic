"""ViT-5-Small + Multi-Head Hyena ImageNet-1k — CLS-row, gated, RoPE.

Extends the multi-head gated Hyena config with 2D RoPE on Q and K before gating.
This adds relative positional bias to the Q*SiLU(K) gate, which previously had
...
- 2D RoPE on Q and K (rope_base=10000.0).
...
def get_config() -> ExperimentConfig:
    """Return the ViT-5-Small + Multi-Head Hyena CLS-row gated + RoPE config."""
