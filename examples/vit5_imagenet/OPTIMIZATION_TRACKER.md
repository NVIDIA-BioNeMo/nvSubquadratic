...
1. Device-cached normalization tensors via `register_buffer`
1. `torch.where`-based blending in `_BatchThreeAugment` (no boolean-index scatter)
1. Vectorised random permutations via `argsort` in `_BatchColorJitter`
1. Fused uint8→float + normalization for validation path (`_fused_val_normalize`, compiled)
1. `_BatchColorJitter` wrapped in `torch.compile`
...
1. bf16 precision for augmentation pipeline
1. Single compiled function for ColorJitter + normalization
...
1. **Validation every 4 epochs** — reduces DALI pipeline interruptions
1. **12 workers** (up from 8) — better utilization of 16 CPU cores/GPU
1. **Prefetch factor 3** (up from 2) — deeper buffer for micro-stall absorption
1. `/scratch` mounted into container via `--container-mounts`
...
1. **The data pipeline is fully optimized**: on NVMe (where fetch ≈ 1-5 ms), the total non-compute overhead is ~3 ms (fetch + augment + optimizer). The 70.4 ms step on NVMe is consistent with: ~2 ms fetch + 0.35 ms augment + ~32 ms fwd + ~34 ms bwd + ~2 ms optim ≈ 70 ms.
1. **Remaining optimization opportunities are compute-side**: DDP allreduce efficiency, batch size scaling, or model-level optimizations — not data loading.
1. **I/O variance across ranks is the dominant source of step-time variance on shared FS**: DALI fetch ranges from 0.6 ms to 200+ ms, causing DDP synchronization stalls. NVMe eliminates this.
...
1. **DALI `fn.transpose` is not free** — It performs an explicit memory copy, adding ~47ms. Keeping HWC→NCHW in PyTorch is faster.
1. **v2 vs v1 augmentation optimizations mattered less than expected** — With `torch.compile`, the augmentation overhead shrank, making the compute-side gains modest. The I/O fix was far more impactful.
1. **Validation frequency matters** — Validating every epoch adds ~3s of overhead per epoch (DALI pipeline rebuild + checkpoint callback). At 52s/epoch, that's ~6% overhead eliminated by going to every 4 epochs.
1. **Container mounts are easy to miss** — `/scratch` must be explicitly mounted in `--container-mounts` for Pyxis/enroot containers.
1. **`torch.compile` warmup needs 15-20 iterations** — Initial profiling with 5-iteration warmup gave misleading results (massive "other overhead").
1. **Fusing augmentations into DALI eliminated the serial bottleneck** — Moving ThreeAugment, ColorJitter, and normalization into the DALI pipeline reduced serial GPU augmentation from ~6 ms (`on_before_batch_transfer`) to 0.35 ms (Mixup + permute only). Further data pipeline optimization yields diminishing returns (\<0.5% of step time).
1. **DDP sync stalls dominate on shared FS** — Per-phase profiling revealed that I/O variance across ranks (0.6–200+ ms) causes DDP allreduce waits, inflating backward time. NVMe eliminates this by providing consistent low-latency reads.
