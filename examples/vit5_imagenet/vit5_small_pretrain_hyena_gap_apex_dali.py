"""ViT-5-Small + Hyena + GAP ImageNet-1k — Apex FusedLAMB + DALI backend.

Identical to vit5_small_pretrain_hyena_gap_apex.py except the dataset
section uses the DALI-accelerated ``ImageNetDALIDataModule`` for GPU-decoded
JPEG loading and GPU augmentations.

Key differences from the non-DALI config:
- Uses ``ImageNetDALIDataModule`` instead of ``ImageNetDataModule``
- ``num_threads`` replaces ``num_workers`` (DALI uses internal threading)
- No ``pin_memory`` / ``prefetch_factor`` (DALI handles memory management)
- Produces channels-last [B, H, W, C] tensors directly on GPU
"""
...
from experiments.datamodules.imagenet_dali import ImageNetDALIDataModule
...
def get_config() -> ExperimentConfig:
    """Return the ViT-5-Small + Hyena (GAP) config with Apex FusedLAMB + DALI backend."""
    config = ExperimentConfig()
    config.debug = False
    config.seed = 42
    config.compile = True
    config.compile_mode = "reduce-overhead"  # max-autotune crashes: Inductor can't codegen complex64 from FFT ops

    # ─── Dataset (DALI) ─────────────────────────────────────────────────
    config.dataset = LazyConfig(ImageNetDALIDataModule)(
