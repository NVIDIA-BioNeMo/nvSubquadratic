...
"""Example Config file for PDE (SWE) regression."""

import os

import torch

from experiments.datamodules.pde.swe import SWEDataModule, PREV_STEPS, EVAL_ROLLOUT_STEPS
from experiments.default_cfg import ExperimentConfig, SchedulerConfig, TrainConfig, WandbConfig
from experiments.lightning_wrappers import PDERegressionWrapper
...
from nvsubquadratic.networks.classification_resnet import ResidualNetwork
...
def get_config() -> ExperimentConfig:
    """Get the configuration for the PDE (SWE) regression experiment.

    Returns:
        ExperimentConfig: The configuration for the PDE (SWE) regression experiment.
    """
    ...
    # Add dataset config
    # Update dataset with LazyConfig directly referencing the SWEDataModule class
    # and providing all parameters directly (no nested params dataclass)
    config.dataset = LazyConfig(SWEDataModule)(
        data_dir=".data/pde/swe",
        data_type=DATA_TYPE,
    ...
    config.net = LazyConfig(ResidualNetwork)(
        in_channels=4, # 2 time steps x (vorticity, pressure)
        out_channels=2, # (vorticity, pressure)
        ...
        block_cfg=LazyConfig(ResidualBlock)(
            sequence_mixer_cfg=LazyConfig(QKVSequenceMixer)(
                ...
                mixer_cfg=LazyConfig(Hyena)(
                    ...
                    apply_qk_norm=True, # TODO: set to False
                    use_rope=True,
                    rope_base=10000.0,
                ),
    ...
    config.lightning_wrapper_class = LazyConfig(PDERegressionWrapper)(
        metric="MSE",
