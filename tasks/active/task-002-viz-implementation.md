...
## Acceptance Criteria
- [ ] **Panel 1 & 2:** `plot_kernel_norm_heatmaps` and `plot_raw_vs_masked` use 99th percentile clipping for `vmax` instead of absolute max.
- [ ] **Panel 4:** `plot_spectral_analysis` clips or masks out the DC component and adds relative frequency axis labels ([-0.5, 0.5] cycles/pixel).
- [ ] **Panel 6:** `plot_activation_maps` uses $log_2(\text{post}/\text{pre})$ for the ratio and centers the `RdBu_r` colormap at 0.
- [ ] **Panel 9:** `plot_kernel_pca` is rewritten to use K-Means clustering (e.g., k=4) instead of PCA to RGB mapping, assigning a distinct categorical color to each spatial position based on its dominant mixing mode.
- [ ] **Panel 10:** `plot_kernel_slices` selects off-diagonal channel pairs dynamically based on highest spatial energy/variance rather than hardcoded indices `(0, hd//4)`.
