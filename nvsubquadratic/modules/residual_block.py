...
class AdditiveCondResidualBlock(torch.nn.Module):
    """Residual block that re-injects an additive conditioning vector at its input.

    Mirrors :class:`ResidualBlock` branch-for-branch, but before the mixer branch it
    broadcast-adds the conditioning vector to the residual stream:

        x <- x + gate * condition[:, None, ..., None, :]

    where ``gate`` is either 1.0 (``cond_gate="none"`` — zero extra params) or
    ``tanh(alpha)`` with a scalar zero-initialised parameter (``cond_gate="scalar_zero_init"``)
    so training starts identical to an unconditional ResNet and the gate opens gradually.

    The mixer/MLP branches never receive the condition tensor themselves:
    ``pass_condition_to_sequence_mixer`` is intentionally not exposed, and callers should
    leave ``condition_mixer_cfg=Identity``. All conditioning flows through the input tap.
    """
    ...
    def __init__(
        ...
        mlp_norm_cfg: LazyConfig,
        dropout_cfg: LazyConfig,
        cond_gate: Literal["none", "scalar_zero_init"] = "scalar_zero_init",
    ):
        """Initialize the AdditiveCondResidualBlock.

        Args mirror :class:`ResidualBlock` except for ``cond_gate``:
            cond_gate: "none" for parameter-free additive re-injection, or
                "scalar_zero_init" for a learned zero-initialised tanh gate.
        """
        if sequence_mixer_cfg.__target__ == torch.nn.Identity:
            assert sequence_mixer_norm_cfg.__target__ == torch.nn.Identity, (
                "Sequence mixer norm must be Identity if sequence mixer is Identity"
        ...
        super().__init__()
        if cond_gate not in ("none", "scalar_zero_init"):
            raise ValueError(f"Unknown cond_gate: {cond_gate}")
        self.cond_gate = cond_gate
        ...
        if cond_gate == "scalar_zero_init":
            # Zero-init scalar gate → additive term vanishes at step 0, matching the
            # unconditional ResNet gradient landscape; opens gradually via tanh.
            self.cond_alpha = torch.nn.Parameter(torch.zeros(1))
            self.cond_alpha._no_weight_decay = True
        else:
            self.register_parameter("cond_alpha", None)

    def forward(self, x: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        """Additively re-inject condition, then run mixer / (optional cross-attn) / MLP branches."""
        if condition is None:
            raise ValueError("AdditiveCondResidualBlock requires a conditioning tensor.")

        # Broadcast condition [B, hidden_dim] (or [B, *spatial_cond, hidden_dim]) over x's spatial dims.
        cond = condition
        if cond.ndim >= 3:
            cond = cond.mean(dim=tuple(range(1, cond.ndim - 1)))  # collapse spatial → (B, hidden_dim)
        while cond.ndim < x.ndim:
            cond = cond.unsqueeze(1)  # add singleton spatial dims

        if self.cond_alpha is not None:
            x = x + torch.tanh(self.cond_alpha) * cond
        else:
            x = x + cond
        ...
        # Cross attention branch (kept for symmetry with ResidualBlock; expected Identity here)
        if not isinstance(self.condition_mixer, torch.nn.Identity):
            residual = x
            x = self.condition_mixer_norm(x)
            x = self.condition_mixer(x, condition)
