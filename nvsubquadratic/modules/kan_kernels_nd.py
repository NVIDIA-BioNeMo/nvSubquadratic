...
Raw grid coordinates in [-1, 1] are fed directly into ``num_layers - 1``
KANLinear (B-spline) layers from warpKAN, followed by a single
``nn.Linear`` output projection ``mlp_hidden_dim -> out_dim``.  No
positional embedding is used.  All computation is in float32 as
...
class KANKernelND(torch.nn.Module):
    """Kernel parameterized by a KAN body followed by a linear projection.

    Raw coordinates in [-1, 1] are passed through ``num_layers - 1``
    KANLinear layers (all ``mlp_hidden_dim``-wide) and a final
    ``nn.Linear(mlp_hidden_dim, out_dim)`` output projection.  No
    ...
    Args:
        ...
        mlp_hidden_dim: Hidden width of all KAN body layers and the input
            of the final linear projection.
        num_layers: Total number of layers (KAN body + output Linear).
            Must be >= 2.  KAN body has ``num_layers - 1`` KANLinear layers,
            followed by a single ``nn.Linear`` output projection.
    ...
    def __init__(
        ...
        # KAN body: (num_layers - 1) KANLinear layers, all mlp_hidden_dim wide.
        # Final projection is a plain nn.Linear(mlp_hidden_dim, out_dim).
        ...
        self.output_proj = torch.nn.Linear(mlp_hidden_dim, out_dim)
