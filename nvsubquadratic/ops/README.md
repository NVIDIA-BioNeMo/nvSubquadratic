...
| ---------------------------------------------------- | --------- | ----------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| [fftconv.py](fftconv.py)                             | fp32      | linear            | depthwise                            | The default. 1D/2D/3D, causal & non-causal.                                                                                                          |
| [circular_fftconv.py](circular_fftconv.py)           | fp32      | circular          | depthwise                            | Periodic boundaries (e.g. PDEs, ARC grids), or when `K = N` so padding is wasteful.                                                                  |
| [fftconv_fp16.py](fftconv_fp16.py)                   | fp16      | linear            | depthwise                            | Memory/throughput savings on power-of-2 spatial dims. Drop-in for `fftconv.py`.                                                                      |
| [circular_fftconv_fp16.py](circular_fftconv_fp16.py) | fp16      | circular          | depthwise                            | Same as above for the circular case. Uses **dual mean-centering** for fp16 stability — see [FP16_FFTCONV_DERIVATION.md](FP16_FFTCONV_DERIVATION.md). |
| [fftconv_chunked.py](fftconv_chunked.py)             | fp32      | linear            | depthwise                            | Memory-constrained training; processes channels in chunks. Has a global flag so models can opt in transparently.                                     |
| [fftconv_multihead.py](fftconv_multihead.py)         | fp32      | linear & circular | dense within head, optional low-rank | Multi-head FFT conv — channel mixing inside each head, in the spirit of multi-head attention.                                                        |
| [fftconv_custom.py](fftconv_custom.py)               | fp32      | linear            | depthwise                            | Wraps an optional fused CUDA kernel (`subquadratic_ops_torch.fft_conv2d`) behind the same API as `fftconv.py`.                                       |
...
- **[`nvsubquadratic/modules/kernels_nd.py`](../modules/kernels_nd.py)** — learned kernel parametrisations that produce the kernels these ops consume.
- **[`nvsubquadratic/modules/hyena_nd.py`](../modules/hyena_nd.py)** — the Hyena operator, the main consumer of these ops.
- **[`nvsubquadratic/modules/ckconv_nd.py`](../modules/ckconv_nd.py)** / **[`ckconv_multihead_nd.py`](../modules/ckconv_multihead_nd.py)** — CKConv variants that compose these primitives.
