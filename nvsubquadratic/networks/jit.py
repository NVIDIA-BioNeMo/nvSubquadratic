...
# SPDX-FileCopyrightText: Copyright (c) 2023 The JiT authors and contributors.
...
#
# Portions of this file are derived from https://github.com/LTH14/JiT
# (MIT License). See LICENSE/third_party.txt for the full MIT notice.
...
def modulate(x, shift, scale):
    """Apply adaptive scale/shift modulation to token features."""
    return x * (1 + scale.unsqueeze(1)) + shift.unsqueeze(1)


class BottleneckPatchEmbed(nn.Module):
    """Image-to-patch embedding with a bottleneck projection."""

    def __init__(self, img_size=224, patch_size=16, in_chans=3, pca_dim=768, embed_dim=768, bias=True):
        """Initialize the bottleneck patch embedding module."""
        super().__init__()
...
class TimestepEmbedder(nn.Module):
    ...
    def __init__(self, hidden_size, frequency_embedding_size=256):
        """Initialize the timestep embedding MLP."""
        super().__init__()
    ...
    def forward(self, t):
        """Embed scalar timesteps into the model hidden space."""
...
class LabelEmbedder(nn.Module):
    ...
    def __init__(self, num_classes, hidden_size):
        """Initialize the class-label embedding table."""
        super().__init__()
        self.embedding_table = nn.Embedding(num_classes + 1, hidden_size)
        self.num_classes = num_classes

    def forward(self, labels):
        """Embed integer class labels."""
        embeddings = self.embedding_table(labels)
        return embeddings


def scaled_dot_product_attention(query, key, value, dropout_p=0.0) -> torch.Tensor:
    """Compute scaled dot-product attention via the native PyTorch op."""
    # Use native flash attention!
    # No need for manual attn_bias of zeros, it just disables flash attention.
    return F.scaled_dot_product_attention(query, key, value, dropout_p=dropout_p)


class Attention(nn.Module):
    """Multi-head self-attention with optional Q/K normalization and RoPE."""
    ...
    def forward(self, x, rope):
        """Apply attention to token sequence ``x`` using the provided RoPE module."""
        B, N, C = x.shape
...
class FinalLayer(nn.Module):
    ...
    def __init__(self, hidden_size, patch_size, out_channels):
        """Initialize the JiT output projection layer."""
        super().__init__()
    ...
    def forward(self, x, c):
        """Project conditioned hidden states back to patch predictions."""
...
class JiTBlock(nn.Module):
    """Transformer block with adaptive layer norm modulation."""

    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, attn_drop=0.0, proj_drop=0.0):
        """Initialize one JiT transformer block."""
        super().__init__()
    ...
    def forward(self, x, c, feat_rope=None):
        """Run one conditioned transformer block pass."""
...
class JiT(nn.Module):
    ...
    # Required for DiffusionWrapper compatibility
    @property
    def hidden_dim(self):
    ...
    def unpatchify(self, x, p):
        """Reconstruct spatial images from patch tokens."""
        c = self.out_channels
    ...
    def forward(self, input_and_condition):
        ...
        # DiffusionWrapper expects BHWC logits, unpatchify returns BCHW
