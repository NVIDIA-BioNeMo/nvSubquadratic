...
#SBATCH --exclusive
#SBATCH --job-name=healthcareeng_research-nvsubq.v5hybridkan1n

# Usage (from repo root):
#   sbatch slurm/submit_hybrid_kan_1node.sh examples/vit5_imagenet/vit5_hybrid/full_hyena_kan.py \
#       net.patch_size=8 dataset.batch_size=64 train.accumulate_grad_steps=4
#
# Patch size / batch / accum guide (1 node = 8 GPUs, effective batch = 2048):
#   patch 8:  batch_size=64  accumulate_grad_steps=4
#   patch 4:  batch_size=16  accumulate_grad_steps=16
#   patch 2:  batch_size=4   accumulate_grad_steps=64
#   patch 1:  batch_size=1   accumulate_grad_steps=256
...
IMAGE_NAME="${SQSH_IMAGE:-/lustre/fsw/portfolios/healthcareeng/projects/healthcareeng_bionemo/amoradzadeh/hyena/nvsubquadratic-slurm-x86_64-04-22-2026-warpkan.sqsh}"

# Host paths
WORKDIR="${PWD}"
RUNS_DIR="${WORKDIR}/runs"
DATA_DIR="${IMAGENET_HOST_DIR:-/lustre/fsw/portfolios/healthcareeng/projects/healthcareeng_bionemo/amoradzadeh/hyena}"
