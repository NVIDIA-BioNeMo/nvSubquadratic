...
2. ~~GPU JPEG decode + GPU transforms~~ — Tested. **2x slower** due to per-image resize overhead.
3. NVIDIA DALI pipeline (truly pipelined GPU decode + transforms)
4. WebDataset / sharded tar format
5. ffcv `.beton` format
6. Pre-decoded dataset on local NVMe
