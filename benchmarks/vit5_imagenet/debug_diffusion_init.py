...
def run_diagnostics():
    ...
    print(
        f"Condition emb: shape={tuple(condition.shape)}, "
    ...
    print(f"\nNoisy image z: mean={z_bchw.mean():.3f}, std={z_bchw.std():.3f}, abs_max={z_bchw.abs().max():.3f}")
    print(f"Target v:      mean={target_v.mean():.3f}, std={target_v.std():.3f}, abs_max={target_v.abs().max():.3f}")
    ...
    x = network.in_proj(x)
    print(
        f"After Patchify:      shape={tuple(x.shape)}, "
    ...
    print(
        f"After cond_in_proj:  shape={tuple(cond.shape)}, "
    ...
    for i, block in enumerate(network.blocks):
        ...
        if i in (0, len(network.blocks) // 2, len(network.blocks) - 1):
            print(f"After block {i:2d}:       mean={x.mean():.3f}, std={x.std():.3f}, abs_max={x.abs().max():.3f}")

    # 5. out_norm
    x = network.out_norm(x)
    print(f"After out_norm:      mean={x.mean():.3f}, std={x.std():.3f}, abs_max={x.abs().max():.3f}")

    # 6. out_proj (Unpatchify)
    x = network.out_proj(x)
    print(
        f"After Unpatchify:    shape={tuple(x.shape)}, "
        f"mean={x.mean():.3f}, std={x.std():.3f}, abs_max={x.abs().max():.3f}"
    )
    ...
    print(f"\nDenominator (1-t):   mean={denominator.mean():.3f}, min={denominator.min():.3f}  (clamped at 0.05)")
    print(
        f"Prediction bchw:     mean={prediction_bchw.mean():.3f}, "
        f"std={prediction_bchw.std():.3f}, abs_max={prediction_bchw.abs().max():.3f}"
    )
    print(
        f"Predicted v:         mean={predicted_v.mean():.3f}, "
        f"std={predicted_v.std():.3f}, abs_max={predicted_v.abs().max():.3f}"
    )
    print(
        f"Target v:            mean={target_v.mean():.3f}, "
    ...
    print(f"  Expected output std from 768 channels of N(0,1): {math.sqrt(w.shape[0]) * w.std().item():.4f}")
