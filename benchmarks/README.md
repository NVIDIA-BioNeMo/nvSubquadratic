...
4. **QuACK fused RMSNorm** — `quack.rmsnorm` replaces the manual float32-upcast-then-downcast RMSNorm with a single fused Triton kernel.
...
8. **Fused DALI augmentations** — ThreeAugment, ColorJitter, and normalization moved entirely into the DALI pipeline using `enable_conditionals=True`. Eliminates ~25ms of serial GPU augmentation per step.

9. **Local NVMe staging** — `prepare_data()` copies ImageNet to node-local `/scratch` with sentinel-based idempotency. Eliminates cross-rank I/O variance that caused DDP allreduce stalls on shared FS.

10. **Training recipe tuning** — Validation every 4 epochs (not every epoch), 12 workers, prefetch_factor=3.
