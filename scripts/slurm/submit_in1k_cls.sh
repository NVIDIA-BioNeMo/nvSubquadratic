...
#SBATCH --exclusive
#SBATCH --job-name=healthcareeng_bionemo-nvsubq.v5patch

# Usage (from repo root):
#   sbatch slurm/submit_in1k_cls.sh examples/vit5_imagenet/v5_patch/hyena_patch16.py
#   sbatch slurm/submit_in1k_cls.sh examples/vit5_imagenet/v5_patch/hyena_patch16.py \
#       dataset.batch_size=128 train.accumulate_grad_steps=2
#
...
# Container image
IMAGE_NAME="${SQSH_IMAGE:-/lustre/fsw/healthcareeng_bionemo/farhadr/enroot/nvsubquadratic-slurm-x86_64.sqsh}"
...
# ImageNet paths (container-side)
export IMAGENET_PATH="${CONTAINER_DATA}/imagenet"
export IMAGENET_FOLDER_PATH="${CONTAINER_DATA}/imagenet"
export LOCAL_STAGING_DIR="${CONTAINER_DATA}/imagenet"
