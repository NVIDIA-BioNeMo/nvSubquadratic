...
# ---------------------------------------------------------------------------
# Normwise error budgets, shared by the fused-kernel test files
# ---------------------------------------------------------------------------


# Calibrated against the observed error of the fused kernel vs the fp32 torch
# reference (fp32 ~3e-7, fp16 ~3e-4, bf16 ~3e-3) with ~3x headroom. Gradients
# accumulate over the batch, so they get more room. Callers needing a looser
# table pass their own via ``tol_table``.
...
def assert_l2_close(pred, ref, dtype, tol_table=None, name=""):
    """Assert ``pred`` matches ``ref`` within the normwise budget for ``dtype``."""
