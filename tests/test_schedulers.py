...
class TestWSDScheduler:
    ...
    def test_wsd_phases(self, optimizer):
        ...
        # Check Initial LR (should be 0.0 if init called step, or 1.0 if not)
        # Verify observed behavior: last_epoch=-1 initially.
        lr_init = optimizer.param_groups[0]['lr']
        # Depending on environment, init might have called step? 
    ...
    def test_no_warmup(self, optimizer):
        scheduler = WSDScheduler(
            optimizer, total_iterations=10, warmup_iterations=0, decay_iterations_percentage=0.2
        )
