...
echo "Test 2/5: Sequence mixer CP equivalency..."
...
# Test 3: MNIST CP (2D)
echo "Test 3/5: MNIST CP integration..."
...
# Test 5: End-to-end gradient equivalence with DataLoader
echo "Test 5/5: E2E gradient equivalence with DataLoader..."
if ! python tests/e2e_distributed_gradient_test.py \
    --config examples/mnist_classification/experiments/mnist_classification_ccnn_testing_minimal_dataset_hyena_rope_qknorm.py \
    --tolerance 0.35 \
    --batch_size 4; then
    echo "ERROR: E2E gradient test failed!"
    FAILED=1
fi
